...
// ===========================
// 🗺️ Kakao Map HTML (JS 키 주입)
// ===========================
app.get('/kakao-map.html', (req, res) => {
  try {
    const key = process.env.KAKAO_JS_KEY || req.query.key || '';
    const filePath = path.join(__dirname, 'public', 'kakao-map.html');

    // 파일 존재 여부 확인
    if (!fs.existsSync(filePath)) {
      return res.status(404).send('kakao-map.html not found');
    }

    const html = fs.readFileSync(filePath, 'utf8').replace(/__KAKAO_JS_KEY__/g, key);
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Cache-Control', 'no-store');
    res.status(200).send(html);
  } catch (e) {
    console.error('❌ kakao-map.html load error:', e.message);
    res.status(500).send('kakao-map.html load error');
  }
});

// ✅ /public 정적 파일 제공 (kakao-map.html 등)
