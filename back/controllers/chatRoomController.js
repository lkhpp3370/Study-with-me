...
/* ===========================
   ✅ 채팅방 알림 On/Off
   - body.enabled 가 boolean이면 그대로 반영
   - 없으면 토글 (이전 동작과 호환)
=========================== */
exports.toggleNotification = async (req, res) => {
  try {
    const { roomId } = req.params;
    const { userId, enabled } = req.body;   // ✅ enabled 수신
    const user = await User.findById(userId);
    if (!user) return res.status(404).json({ error: '유저 없음' });

    if (!user.chatNotificationPreferences) {
      user.chatNotificationPreferences = new Map();
    }
    const current = user.chatNotificationPreferences.get(roomId);
    const next = (typeof enabled === 'boolean') ? enabled : !(current === true);
    user.chatNotificationPreferences.set(roomId, next);
    await user.save();
    res.json({ roomId, notifyEnabled: next });
  } catch (err) {
...
/* ===========================
   ✅ 채팅방 단건 정보 조회
   - 멤버수, pinnedMessage, pinnedHistory 길이 등
=========================== */
exports.getRoomInfo = async (req, res) => {
  try {
    ...
    const room = await ChatRoom.findById(roomId)
      .populate("members", "username")
      .populate("pinnedMessage", "_id type content createdAt")
      .populate("lastMessage", "_id type content createdAt");

    if (!room) return res.status(404).json({ error: "채팅방 없음" });

    // 스터디 멤버수 기준이 필요하면 Study 조회
    let studyMemberCount = 0;
    if (room.studyId) {
      const study = await Study.findById(room.studyId).select("members title");
      studyMemberCount = study?.members?.length || 0;
    }
    ...
    return res.json({
      ...
      memberCount: studyMemberCount || room.members.length,
      pinnedMessage: room.pinnedMessage || null,
      pinnedHistoryCount: (room.pinnedHistory || []).length,
      lastMessage: room.lastMessage || null,
      lastMessageAt: room.lastMessageAt || null,
    });
  } catch (err) {
    console.error("❌ 채팅방 정보 조회 실패:", err);
    return res.status(500).json({ error: "채팅방 정보 조회 실패" });
  }
};